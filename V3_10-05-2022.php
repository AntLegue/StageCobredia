...
<?php
setcookie("QUALITY","80", time()+699999); // 30 jours de validit�
...
if ( $_POST['vehicule_id'] != '-' )
	{	
	setcookie("VEHICULE_ID", $_POST['vehicule_id'], time()+699999); // 30 jours de validit�
...
if ( $_POST['immatriculation'] != '-' && $_POST['immatriculation'] != '')
	...
	setcookie("VEHICULE_ID", $id[1], time()+699999); // 30 jours de validit�
...
if ( $_POST['connecter'] == 'Connecter' )
	...
	if ( $row[0] != '' )
		...
		$token=passgen2(30);
		//echo "set : ".$value;
		//setcookie("CAPVO_TOKEN", $token, time()+2592000); // 30 jours de validit�
		setcookie("CAPVO_TOKEN", $token, time()+699999); // 30 jours de validit�
...
//echo "QUAL :" .$_COOKIE['QUALITY']; // 30 jours de validit�
?>
<script>
...
$(document).ready(function()
    ...
    var imagesPreview = function(input, placeToInsertImagePreview,fic_type,order) 
		{
			if (input.files) 
			{
				...
				for (i = 0; i < filesAmount; i++) 
				{
					...
					// D�fini les actions � effectuer apr�s chargement du contenu du fichier
					// s�lectionn� dans le input par l'utilisateur.
					...
					// Charge le contenu du fichier s�lectionn� dans le input par l'utilisateur.
	...
	//fonction d'affichage des images et camoufles celle d�j� affich�e
	/**
	 * Fonction d'affichage des photos dans les divs au chargement de ces dernières
	 * @event - onChange 
	 * @return {none}
	 */
	function update_preview(event) {
		...
		//affiche la div pour l"image a charg�		
		...
		//cache le chargement termin�e au bout de 3s
	...
	//fonction sur clic bouton pour revoir la photo charg�e
	/**
	...
	function replay_photo(event){
		...
		//v�rifie si la div est affich�e
...
{
require("../config.php");
// Cr�ation de la demande de cotation
// Update des infos : les images ont �t� pr�charg� en javascripts
//
...
// supression du d�calage de mois dans mreprise
...
file_put_contents($logfilew,$sql."\r\n",FILE_APPEND);
$date=date('d/m/y');
// Cr�ation de l'offre sous forme HTML	
$html="
...
{
require("../config.php");
// r�cup�ration des infos du vendeur, nom, prenom concession	
...
<tr class="<?= $tr_classes ?>">
	...
	<td align="center">
		<label for="photo_<?= $id ?>" class="label-file">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;<img id="Photo_loading_<?= $id ?>" src="/dcapvo/img/bouton-loading/<?= $infos["photo_name"]?>" width="100px" height="auto">
		</label>
