...
if ( $_COOKIE['CAPVO_WEBAPP_VERSION'] == '' && $_COOKIE['DEVICE'] == 'android' ) { 
	...
	<tr>
		<td colspan='2' height='20' bgcolor='#DDDDDD'>
			...
			<br>T&eacute;l&eacute;charger le fichier puis aller dans le dossier t&eacute;l&eacute;chargement et cliquer sur le fichier app-capvo.apk<br>En vous remerciant<br>L'&eacute;quipe CAPVO<br><br>
		</td>
	</tr>
	<?php
	}
			...
			<table border='0' cellspacing='0' cellpadding='0'>
				...
				<tr>
					<td height='25' width='345' colspan='2'  style='background-color: #EEEEEE'>
						&nbsp;N&deg; : 
						<select id='vehicule_id' name='vehicule_id' onchange="redirm('vehicule_id')" width='300'>
				...
				<tr>
					<td height='40' width='150' colspan='2'  style='background-color: #EEEEEE'>
						&nbsp;Immatriculation : 
						<input id='immatriculation' autocomplete='off' type='text' name='immatriculation' placeholder='Saisir Immatriculation'>
							&nbsp;&nbsp;
...
<tr>
	...
	<td>
		<a href='#' onclick="showPhotos(0);" style='color:white;background-color:#FF7F00;text-decoration:none;font-size:14px;padding:4px'>
...
<tr>
	<td>
		...
		<?php
			/* 
			$sql = $pdo->query("select coteur_email from coteurs where coteur_nom = {$vehicule_row[22]};");
			$mail = $sql->fetch();
			echo $mail;*/
		?>		
	</td>
...
-->
<?php
    /*$retour = mail('destinataire@free.fr', 'Informations Complémentaire', $_POST['message'], 'From: webmaster@monsite.fr');
    if ($retour)
...
if ( $statut_couv == 'open' && $newest_couverture_row[0] != $nbcouverture)
	{
		?>
			<tr bgcolor="#B0F2B6">
				...
				<td>
					<a href="#" onclick="showCvtGrid(0)" style="color:white;background-color:#FF7F00;text-decoration:none;font-size:14px;padding:4px">
...
if ( $_COOKIE['CAPVO_WIDTH'] <= 400 )
	{
	?>
		<div id="photos" style="max-height:665px;background-color:white;display: none;position: absolute;top: 10px; left:360px; z-index: 99;overflow-y:scroll;box-shadow: 8px 8px 12px #aaa;border: solid #3D3232 1px; width: 35%;">
			<button href='#' class="Position" onclick="showPhotos(1); refreshPage();" style="color:black;font-size:15px;text-decoration:none;color:black;font-size:18px;">
				<b>X</b>
			</button>
	<?php
	}
		...
		if ($typefile[1] == 'avant' || $typefile[1] == 'droite' || $typefile[1] == 'gauche' || $typefile[1] == 'arriere' ||  $typefile[1] == 'int1' || $typefile[1] == 'int2' ||$typefile[1] == 'car1' ||$typefile[1] == 'car2' ||$typefile[1] == 'car3' ||$typefile[1] == 'car4' ||$typefile[1] == 'car5' ||$typefile[1] == 'car6' ||$typefile[1] == 'car7' ||$typefile[1] == 'car8' ||$typefile[1] == 'car9' ||$typefile[1] == 'car10' ||$typefile[1] == 'ct' || $typefile[1] == 'reprise' || $typefile[1] == 'reprisecapvo' )
			...
			if ( $atype[0] == "image" && ($typefilejpg[1] == "jpg" || $typefilejpg[1] == "jpeg" || $typefilejpg[1] == "png" ||  $typefilejpg[1] == "PNG" || $typefilejpg[1] == "JPG" || $typefilejpg[1] == "jfif") && $typefilejpg[2] =='' )
				...
				if ( file_exists ( $dirdoc.$vehicule_id."/{$entry}.jpgl" ) )
					{
						?>
							<img class="zoomD" src="/<?=$version?>/doc/<?=$vehicule_id?>/<?=$entry?>.jpgl" style="max-width:100%;max-height:auto;float:left;background-size: cover;padding:0.5%" loading="lazy"><br></img>							
						<?php
					}
				else
					{
						?>
							<img class="zoomD" src="/<?=$version?>/doc/<?=$vehicule_id?>/<?=$entry?>" style="max-width:100%;max-height:auto;float:left;background-size: cover;padding:0.5%" loading="lazy"></img>		
						<?php
					}	
				}
				...
				if ( $_COOKIE['CAPVO_WIDTH'] <= 400 )
					{
						?>
							<embed src="/<?=$version?>/doc/<?=$vehicule_id?>/<?=$entry?>" style="max-width:100%;float:left;" type="application/pdf">
						<?php
					}
				else
					{
						?>
							<embed src="/<?=$version?>/doc/<?=$vehicule_id?>/<?=$entry?>" width='840px' height='520px' style="max-width:100%;float:left;" type="application/pdf">
						<?php
					}
				}
			}	
...
if ( $statut_couv == 'open' && $newest_couverture_row[0] != $nbcouverture)
	{
	?>
		<div id="price_cont" style="background-color:white;display: none;overflow:hidden;position: absolute;left: 8px;top: 230px;z-index: 99">
    		<table width="321px" height="185px" style="border: solid #3D3232 1px; -moz-box-shadow: 8px 8px 12px #aaa; box-shadow: 8px 8px 12px #aaa; -webkit-box-shadow: 8px 8px 12px #aaa;">
        		<tr height='25'>
            		<td><input type="button" name="add" value="Ajouter une couverture" id='addcvt' style='color:white;background-color:#FF7F00;text-decoration:none;font-size:12px;padding:2px' onclick="var id=mygrid_price.uid(); mygrid_price.addRow(id,'');"></td>
            		<td><a href="#" onclick="showCvtGrid(1);return false;" style='position: absolute;right: 5px;top: 6px;color:white;background-color:#FF7F00;text-decoration:none;font-size:12px;padding:2px'>Valider l'offre</a></td>
        		</tr>
        		<tr height='25'>
            		<td colspan='2' valign='top' align='right' style='color:black;font-size:12px'><b>Palier de Couverture 100 &euro;</b></td>
        		</tr>
        		<tr>
            		<td colspan='2' valign='top'>
                		<div id="price" width="316px" height="132px" style="background-color:white;overflow:hidden"></div>
            		</td>
        		</tr>
    		</table>
		</div>
	...
	?>
		<div id="price_cont" style="background-color:white;display: none;overflow:hidden;position: absolute;left: 8px;top: 230px;z-index: 99">
    		<table width="321px" height="185px" style="border: solid #3D3232 1px; -moz-box-shadow: 8px 8px 12px #aaa; box-shadow: 8px 8px 12px #aaa; -webkit-box-shadow: 8px 8px 12px #aaa;">
        		<tr height='25'>
            		<td><br></td>
            		<td><a href="#" onclick="showCvtGrid(1);return false;" style='position: absolute;right: 5px;top: 6px;color:white;background-color:#FF7F00;text-decoration:none;font-size:12px;padding:2px'>Fermer</a></td>
        		</tr>
        		<tr>
...
$form_images = array(
	...
	"rep" => array(
		"title" => "Estimation",
		"photo_name" => "estimation.jpg"
	),
	"car1" => array(
		"title" => "Divers 1",
		"show_next" => true,
		"photo_name" => "divers.jpg"
	),
	"car2" => array(
		"title" => "Divers 2",
		"show_next" => true,
...
<tr class="<?= $tr_classes ?>">
	<td height="45">
		<?= $infos["title"] ?>: &nbsp;
		<button type="button" class="BoutonPhoto hidden style-bouton" data:field_id="<?= $id ?>" id="<?= $id ?>-bt_photo">
		</button>
	</td>
	...
	<td align="center">
		<label for="photo_<?= $id ?>" class="label-file">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;<img class="HoverZoom" id="Photo_loading_<?= $id ?>" src="/dcapvo/img/bouton-loading/<?= $infos["photo_name"]?>" width="100px" height="auto">
		</label>
