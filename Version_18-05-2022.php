...
<head>
    ...
    * {
    ...
    .style-bouton {
        ...
        float: right;
        /*Donne une taille fixe au bouton*/
        /*width: 22px;*/
...
<script>
//Function to display fullscreen link the css on image click
window.onload = () => {
    // (A) GET LIGHTBOX & ALL .ZOOMD IMAGES
    let all = document.getElementsByClassName("zoomD"),
        fullscreenDiv = document.getElementById("fullscreenDiv");
    ...
    if (all.length > 0) {
        for (let i of all) {
            i.onclick = () => {
                ...
                fullscreenDiv.className = "show";
                //hide scrollbar window
    ...
    fullscreenDiv.onclick = () => {
        ...
        //show scrollbar window
        document.body.style.overflow = 'scroll';
    };
};
</script>

<!-- Div where image is display fullscreen -->
<div align="center" id="fullscreenDiv"></div>
...
if ( $_POST['connecter'] == 'Connecter' )
	...
	$stm=$pdo->query($sql) ; 
	$vendeur_row=$stm->fetch(); 
	//echo 'RET :'. $vehicule_row[0];
	if ( $vendeur_row[0] != '' )
		{
		$token=passgen2(30);
		//echo "set : ".$value;
		...
		if ( $vendeur_row[0] != '' )
			...
			setcookie("CAPVO_ID", $vendeur_row[1], time()+699999);
		 
			$_COOKIE['CAPVO_TOKEN']=$token;
			$_COOKIE['USER_PROFILE']='MARCHAND';
			$_COOKIE['CAPVO_ID']=$vendeur_row[1];
...
function getPrice(id_veh) {
    ...
    document.getElementById("prixpart").innerHTML =
        "<span style='font-family:arial,serif;font-size:10px'>Interrogation en cours ...</span>";
    ...
    request.onload = function(e) {
        ...
        var value = ret.split('|');
        document.getElementById("prixpart").innerHTML =
            "<span style='font-family:arial;font-size:10px'><b>Prix du march&eacute;</b> : " + value[0] +
            " &euro;<br><b> Rotation de vente :</b> " + value[1] + " jours</span>";
...
function plus(id, prix) {
    ...
    if (id == 'fraisjante' && document.getElementById(id).value > 4) {
        document.getElementById(id).value = Number(4);
    }
    //if ( id == 'fraisjantebt' && document.getElementById(id).value > 4 ) { document.getElementById(id).value =Number(4); }
    if (id == 'fraisembrayage' && document.getElementById(id).value > 1) {
        document.getElementById(id).value = Number(1);
    }
    if (id == 'fraisdistribution' && document.getElementById(id).value > 1) {
        document.getElementById(id).value = Number(1);
    }
    if (id == 'fraisrevision' && document.getElementById(id).value > 1) {
        document.getElementById(id).value = Number(1);
    }
    if (id == 'fraispneu' && document.getElementById(id).value > 4) {
        document.getElementById(id).value = Number(4);
    }
    if (id == 'fraisparebrise' && document.getElementById(id).value > 1) {
        document.getElementById(id).value = Number(1);
    }

    document.getElementById(id + "t").value = Number(document.getElementById(id).value) * Number(prix);
    document.getElementById('frais').value = Number(document.getElementById('fraiscarrosseriet').value) + Number(
            document.getElementById('fraisjantet').value) + Number(document.getElementById('fraisrevisiont').value) +
        Number(document.getElementById('fraisembrayaget').value) + Number(document.getElementById('fraisdistributiont')
            .value) + Number(document.getElementById('fraispneut').value) + Number(document.getElementById(
            'fraisparebriset').value) + Number(document.getElementById('fraisnettoyaget').value) + Number(document
            .getElementById('fraisdivers').value);
...
function sendData(data, url, fichier, cotid, fic_type) {
    ...
    xhr.onload = function(evt) {
        ...
        if (fic_type == 'avant' || fic_type == 'arriere' || fic_type == 'gauche' || fic_type == 'droite' ||
            fic_type == 'int1' || fic_type == 'int2' || fic_type == 'cg' || fic_type == 'rep') {
            active_button--;
    ...
    xhr.addEventListener("error", function(
        evt) { //alert ('Erreur lors du transfert de la photo, Merci de reprendre la photo');
        document.getElementById(fic_type).innerHTML =
            "<FONT SIZE='2'>Chargement en erreur : nouvelle tentative</FONT>";
        pausecomp(5000);
        //alert('fini');
        var xhr1 = new XMLHttpRequest();
        xhr1.onload = function(evt) {
            document.getElementById(fic_type).innerHTML = "<FONT SIZE='2'>Chargement termin&eacute;</FONT>";
            if (fic_type == 'avant' || fic_type == 'arriere' || fic_type == 'gauche' || fic_type ==
                'droite' || fic_type == 'int1' || fic_type == 'int2' || fic_type == 'cg' || fic_type ==
                'rep') {
                active_button--;
                console.log(active_button);
                check_val();
...
$(document).ready(function() {
    var imagesPreview = function(input, placeToInsertImagePreview, fic_type, order) {
        if (input.files) {
            ...
            for (i = 0; i < filesAmount; i++) {
                ...
                // Défini les actions é effectuer aprés chargement du contenu du fichier
                // sélectionné dans le input par l'utilisateur.
                reader.addEventListener('load',
                    function(event) {
                        data = reader.result;
                        sendData(data, './save-img.php', nom, cotid, fic_type);
                        // Display a preview of the chosen (uploaded) image in the given HTML element (div.Previsualisation).
                        placeToInsertImagePreview.append(`<img src="${event.target.result}" />`);

                        //Enlever les commentaires pour afficher l'image dans le bouton loading
                        //Update the label button image to display the preview.s
                        $(`tr.block_${input.id} label img`).attr('src', event.target.result);
                    }
                );

                // Charge le contenu du fichier sélectionné dans le input par l'utilisateur.
    ...
    //$('tr.block-image-field input.image-input').change(removeHoverZoom);

    /**
     * Des photos supplementaires peuvent etre ajoutes, cette fonction permet de gerer l'affichage des photos a rajouter 
     * @event - onChange
     * @return {none}
     */
    ...
    //fonction d'affichage des images et camoufles celle déjé affichée
    /**
     * Fonction d'affichage des photos dans les divs au chargement de ces derniÞres
     * @event - onChange 
     * @return {none}
     */
    function update_preview(event) {
        ...
        //recupere le label o¨ est stocké l'image
        var label_photo = $("tr.block_" + input_id + " label.label-file img");
        ...
        //affiche "Fermer la photo sur le bouton"
        bouton_info.html("-");
        ...
        //retire la classe HoverZoom
        label_photo.removeClass("HoverZoom");
    ...
    //lors d'un changement sur un element contenant la class show-next execute fonction show_next
    $('tr.block-image-field input.show-next').change(show_next);

    //fonction sur clic bouton pour revoir la photo chargée
    /**
     * Fonction qui permet de reafficher la photo presente dans une div masquee
     * @event - onChange
     */
...
?>
    ...
    <table width='344' border='0' cellspacing='0'>
        ...
        <tr>
            <td colspan='2' height='70' align='center' bgcolor='#DDDDDD'>
                <img src='/capvo/img/logo.png' width='80'></img>
                &nbsp;&nbsp;
                <img src='/capvo/img/logo-1024t.png' width='222'></img>
            </td>
        </tr>
        <tr>
            <td colspan='2' height='15' bgcolor='#DDDDDD'>
                <div id='msg'><?=$msg?></div>
            </td>
        </tr>
...
if ( $_COOKIE['CAPVO_TOKEN'] !='')
	...
	?>
        <tr>
            <td height='15' bgcolor='#DDDDDD'>&nbsp;&nbsp;
                <a href='#' onClick='deconnect();'>
                    Se d&eacute;connecter
                </a>
            </td>
            <td height='20' bgcolor='#DDDDDD' align='right'>
                <a href='./app-capvo.apk'>
                    Application
                </a>&nbsp;&nbsp;&nbsp;
            </td>
        </tr>
...
if ( $_COOKIE['CAPVO_WEBAPP_VERSION'] == '' && $_COOKIE['DEVICE'] == 'android' ) { 
	?>
        <tr>
            <td colspan='2' height='20' bgcolor='#DDDDDD'>
                <a href='./app-capvo.apk'>
                    Merci de mettre &agrave; jour votre application en cliquant sur ce lien
                </a><br><br>-- Correctif : Compression des photos prisent par l'application<br>
                <br>T&eacute;l&eacute;charger le fichier puis aller dans le dossier t&eacute;l&eacute;chargement et
                cliquer sur le fichier app-capvo.apk<br>En vous remerciant<br>L'&eacute;quipe CAPVO<br><br>
            </td>
        </tr>
        <?php
	}
...
if ( $_COOKIE['USER_PROFILE'] == 'VENDEUR' )
	...
	?>
        <tr>
            <td height="5" bgcolor="#DDDDDD" align="right" colspan="2"></td>
        </tr>
        <tr>
            <td height="20" bgcolor="#DDDDDD" align="right"></td>
            <td bgcolor="#DDDDDD"><?="{$vendeur_row[1]} {$vendeur_row[0]}"?></td>
        </tr>
        <?php	
	...
	?>
        <tr>
            <td height="40" width="172" align="center" bgcolor="<?=$color1?>">
                <a href="#" onclick="document.location.href='./index.php?f=new';">
                    Nouvelle Cotation
                </a>
            </td>
            <td height="40" width="172" align="center" bgcolor="<?=$color2?>">
                <a href="#" onclick="document.location.href='./index.php?f=lst';">
                    Liste des Cotations
                </a>
            </td>
        </tr>
	...
	if ( $vendeur_row[0] == '' )
        ...
        <tr>
            <td height="20" bgcolor="#DDDDDD" align="right">Soci&eacute;t&eacute; :</td>
            <td bgcolor="#DDDDDD"><?=$vendeur_row[0]?></td>
        </tr>
		...
		/*if ($_GET['f'] == 'lstcvt' ||
			...
			?>
        <tr>
            <td height="40" width="172" style="text-align: center; background-color: #DDDDDD;">
                <a href="./index.php?f=lstcvt" id="lstcvt">
...
if ( $_POST['encours'] == 'on' ) 
	...
	?>
                <form name="form_marchand" id="form_marchand" method="post">
                    <table border="0" cellspacing="0" cellpadding="0">
                        <tr>
                            <td height="25" width="345" style="background-color: #EEEEEE" colspan="2">
                                &nbsp;V&eacute;hicules retenus :
                                <input type="checkbox" name="retenu" onClick="redirm('vehicule_id')" <?= $check ?>>
                                </input>
                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;En cours :
                                <input type="checkbox" name="encours" onClick="redirm('vehicule_id')" <?= $check1 ?>>
                                </input>
                            </td>
                        </tr>
                        <tr>
                            <td height="25" width="345" colspan="2" style="background-color: #EEEEEE">
                                &nbsp;N&deg; :
                                <select id="vehicule_id" name="vehicule_id" onchange="redirm('vehicule_id')"
                                    width="300">
                                    <?=lstm($pdo,"vehicule_id",$vehicule_id,"","","", $check,$check1)?>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            <td height="40" width="150" colspan="2" style="background-color: #EEEEEE">
                                &nbsp;Immatriculation :
                                <input id="immatriculation" autocomplete="off" type="text" name="immatriculation"
...
$stm2=$pdo->query($sql2);
$newest_couverture_row =$stm2->fetch();
...
//$start_date = new DateTime($vehicule_row[18].' 19:00:00');
...
?>
        <tr>
            <td colspan="3">
                <table style='background-color: #EEEEEE' border='0' cellspacing='0' cellpadding='4'>
            </td>

        <tr style='background-color:#FFFFFF' height='2'>
            <td colspan="2"></td>
        </tr>

        <tr>
            <td height='35' width='160'><b>
                    N&deg; Dossier :
                </b></td>
            <td width='184'>
                <?= $vehicule_row[0] ?>
        ...
        <!--<tr>
            ...
            <td>
                ...
                <form id="contactForm" action="post.php" method="post">
                    <input id="nom_contact" type="text" placeholder="Entrez votre nom" required>
                    <input id="email_contact" type="email" placeholder="Entrez votre mail" required>
                    <input id="message_contact" type="text" placeholder="Entrez votre message" required>
        ...
        <tr style='background-color:#90C8E1'>
            <td height='35' colspan="2">
                <b>
                    Frais Estim&eacute;s
                </b>
            </td>
        </tr>
        <tr style='background-color:#90C8E1'>
            ...
            <td>
                <?= $vehicule_row[15] ?> &euro;
            </td>
        </tr>
        <tr style='background-color:#90C8E1'>
            <td height='60' valign='top'>
                Commentaire <br>coteur :
            </td>
            <td>
                <?= $row6[0] ?>
            </td>
        ...
        <?php
//echo "D TOMO : ".$vehicule_row[24];
/*
if ( $vehicule_row[24] == '0' ){

	$start_date = new DateTime($vehicule_row[25].' 19:00:00');
...
//Switch to know if it's saturday or sunday
switch ($vehicule_row[24]) {
	...
	//case 6 => Jour Samedi 
    case 6:
		$jourplus2 = date("Y-m-d", strtotime($vehicule_row[25].'+ 2 days'));
        $start_date = new DateTime($jourplus2.' 19:00:00');
        break;
	//case default => Tous les autres jours
	default:
		$start_date = new DateTime($vehicule_row[18].' 19:00:00');
		break;
}
...
$since_start = $start_date->diff(new DateTime($vehicule_row[19]));
...
if ( $vehicule_row[13] <= $nbjcouverture &&  $vehicule_row[17] !=0 )
	{
	//	echo "<tr><td height='25' >Temps restant  :  </td><td>{$minutes } minutes </td></tr>";
		$tpsrestant=($nbcouverture-$vehicule_row[13]);
		if ( $tpsrestant== '1' )
			{
			if ( $time1 < 1900 )
				{
...
if ( $statut_couv == 'open' && $newest_couverture_row[0] != $nbcouverture)
	{
		?>
        <tr bgcolor="#B0F2B6">
            <td height="35">
                <b>Couverture Marchand</b>
            </td>
            <td>
                <a href="#" onclick="showCvtGrid(0)"
                    style="color:white;background-color:#FF7F00;text-decoration:none;font-size:14px;padding:4px">
...
echo "<tr  bgcolor='#B0F2B6'><td height='25'>Mnt couverture : </td><td>".$row3[0]. " &euro;</td></tr>{$htmltpsrestant}
	...
	<tr  bgcolor='#B0F2B6'><td height='25' >Nombre :  </td><td>{$newest_couverture_row[0]}</td></tr>";

//modif TJO 

echo "<tr bgcolor='#B0F2B6'><td height='25' colspan=\"2\"><br>Champs d'information pour les marchands :</td></tr>
	<tr bgcolor='#B0F2B6'><td height='25' colspan=\"2\" ><textarea name='marchandmemo' id='marchandinfo' onChange='updMemMarch(this.value);' rows='3' cols='43'>{$vehicule_row[23]}</textarea> </td></tr>
	</table>";

?>

        <script>
        //permet de relancer la page
...
if ( $_COOKIE['CAPVO_WIDTH'] <= 400 )
	...
	?>
        <div id="photos"
            style="max-height:665px;background-color:white;display: none;position: absolute;top: 10px; left:360px; z-index: 99;overflow-y:scroll;box-shadow: 8px 8px 12px #aaa;border: solid #3D3232 1px; width: 35%;">
            <button href='#' class="Position" onclick="showPhotos(1); refreshPage();"
                style="color:black;font-size:15px;text-decoration:none;color:black;font-size:18px;">
                <b>X</b>
            </button>
            <?php
	}
		...
		if ($typefile[1] == 'avant' || $typefile[1] == 'droite' || $typefile[1] == 'gauche' || $typefile[1] == 'arriere' ||  $typefile[1] == 'int1' || $typefile[1] == 'int2' ||$typefile[1] == 'car1' ||$typefile[1] == 'car2' ||$typefile[1] == 'car3' ||$typefile[1] == 'car4' ||$typefile[1] == 'car5' ||$typefile[1] == 'car6' ||$typefile[1] == 'car7' ||$typefile[1] == 'car8' ||$typefile[1] == 'car9' ||$typefile[1] == 'car10' ||$typefile[1] == 'ct' || $typefile[1] == 'reprise' || $typefile[1] == 'reprisecapvo' )
			...
			if ( $atype[0] == "image" && ($typefilejpg[1] == "jpg" || $typefilejpg[1] == "jpeg" || $typefilejpg[1] == "png" ||  $typefilejpg[1] == "PNG" || $typefilejpg[1] == "JPG" || $typefilejpg[1] == "jfif") && $typefilejpg[2] =='' )
				...
				if ( file_exists ( $dirdoc.$vehicule_id."/{$entry}.jpgl" ) )
					{
						?>
                <img class="zoomD" src="/<?=$version?>/doc/<?=$vehicule_id?>/<?=$entry?>.jpgl"
                    style="max-width:100%;max-height:auto;float:left;background-size: cover;padding:0.5%"
                    loading="lazy"><br></img>
                <?php
					}
				else
					{
						?>
                <img class="zoomD" src="/<?=$version?>/doc/<?=$vehicule_id?>/<?=$entry?>"
                    style="max-width:100%;max-height:auto;float:left;background-size: cover;padding:0.5%"
                    loading="lazy"></img>
                <?php
					}	
				...
				if ( $_COOKIE['CAPVO_WIDTH'] <= 400 )
					{
						?>
                <embed src="/<?=$version?>/doc/<?=$vehicule_id?>/<?=$entry?>" style="max-width:100%;float:left;"
                ...
                <?php
					...
					{
						?>
                <embed src="/<?=$version?>/doc/<?=$vehicule_id?>/<?=$entry?>" width='840px' height='520px'
                    style="max-width:100%;float:left;" type="application/pdf">
                <?php
					}
				}
			}	
...
if ( $statut_couv == 'open' && $newest_couverture_row[0] != $nbcouverture)
	...
	?>
            <div id="price_cont"
                style="background-color:white;display: none;overflow:hidden;position: absolute;left: 8px;top: 230px;z-index: 99">
                <table width="321px" height="185px"
                    style="border: solid #3D3232 1px; -moz-box-shadow: 8px 8px 12px #aaa; box-shadow: 8px 8px 12px #aaa; -webkit-box-shadow: 8px 8px 12px #aaa;">
                    <tr height='25'>
                        <td><input type="button" name="add" value="Ajouter une couverture" id='addcvt'
                                style='color:white;background-color:#FF7F00;text-decoration:none;font-size:12px;padding:2px'
                                onclick="var id=mygrid_price.uid(); mygrid_price.addRow(id,'');"></td>
                        <td><a href="#" onclick="showCvtGrid(1);return false;"
                                style='position: absolute;right: 5px;top: 6px;color:white;background-color:#FF7F00;text-decoration:none;font-size:12px;padding:2px'>Valider
                                l'offre</a></td>
                    </tr>
                    <tr height='25'>
                        <td colspan='2' valign='top' align='right' style='color:black;font-size:12px'><b>Palier de
                                Couverture 100 &euro;</b></td>
                    </tr>
                    <tr>
                        <td colspan='2' valign='top'>
                            <div id="price" width="316px" height="132px" style="background-color:white;overflow:hidden">
                            </div>
                        </td>
                    </tr>
                </table>
            ...
            <div id="price_cont"
                ...
                <table width="321px" height="185px"
                    ...
                    <tr height='25'>
                        <td><br></td>
                        <td><a href="#" onclick="showCvtGrid(1);return false;"
                                style='position: absolute;right: 5px;top: 6px;color:white;background-color:#FF7F00;text-decoration:none;font-size:12px;padding:2px'>Fermer</a>
                        </td>
                    </tr>
                    <tr>
                        <td colspan='2' valign='top' align='right' style='color:black;font-size:12px'>&nbsp;</td>
                    </tr>
                    <tr height='25'>
                        <td colspan='2' valign='top'>
                            <div id="price" width="316px" height="132px" style="background-color:white;overflow:hidden">
...
//$immatriculation_row=$stm->fetch();
//lst($pdo,$type,$value)
while (($immatriculation_row = $stm->fetch()) !== false)
		{
		
		echo "<tr><td colspan='3'><table border='0' cellspacing='0.2' cellpadding='5' bgcolor='#EEEEEE'>
	<tr><td height='25' width='172'>N&deg; Dossier : </td><td width='172'>{$immatriculation_row[7]}</td></tr>
	<tr><td height='25' >Immatriculation : </td><td>{$immatriculation_row[0]}</td></tr>
	<tr><td height='25' >Marque : </td><td>{$immatriculation_row[1]}</td></tr>
	<tr><td height='25' >Modele : </td><td>{$immatriculation_row[2]}</td></tr>
	<tr><td height='25' >Finition : </td><td>{$immatriculation_row[3]}</td></tr>
	<tr><td height='25' >Statut : </td><td><select id='statut_id' name='statut_id' onchange=\"updStatut(this.value,'updst','{$immatriculation_row[7]}')\">".lst($pdo,'statut',$immatriculation_row[4],$immatriculation_row[7],$immatriculation_row[5],$immatriculation_row[6])."</select></td></tr>
	<tr><td height='25' >Date Demande : </td><td>{$immatriculation_row[8]}</td></tr>
	<tr><td height='25' >Coteur : </td><td>{$immatriculation_row[9]}</td></tr>
	<tr><td height='25' >Km liv finale : </td><td><input type='number' id='km' onchange=\"updStatut(this.value,'updkm','{$immatriculation_row[7]}')\" value='{$immatriculation_row[10]}'></input></td></tr>
	<tr><td height='25' >Date liv finale : </td><td><input type='date' id='datelivraison' onchange=\"updStatut(this.value,'upddl','{$immatriculation_row[7]}')\" value='{$immatriculation_row[11]}'></input></td></tr>
	</table> <br>
	</td></tr>";
		}
...
while (($token_row = $stm->fetch()) !== false)
		{
		$token = $token_row[0];
		}	
...
require("../config.php");
$sql="select v.vehicule_id,
    c.concession_nom,
    ve.vendeur_nom,
    co.coteur_nom,
    st.statut_libelle,
    date_format(date_offre, \"%d/%m/%Y\") date_offre,
    immatriculation,
    date_format(date_miseenservice, \"%d/%m/%Y\") date_miseenservice,
    v.marque_nom,
    v.modele_nom,
    motorisation_nom,
    v.finition_nom,
    ...
    mnt_frais,
    origine,
    date_format(date_livraison, \"%d/%m/%Y\") date_livraison,
    mnt_bca,
    mnt_capvo,
    mnt_marchand,
    acheteur_nom,
    mnt_marche,
    vendeur,
    date_format(date_vente, \"%d/%m/%Y\") date_vente,
    date_envoieoffre,
    vendeur_email,
    coteur_email,
    ve.vendeur_id vehicule_link,
    mnt_autobiz
from vehicule_couverts v,
    concessions c,
    vendeurs ve,
    coteurs co,
    statut_cotation st,
    motorisation_vehicule m
where c.concession_id = v.concession_id
    and co.coteur_id = v.coteur_id
    and st.statut_id = v.statut_id
    and ve.vendeur_id = v.vendeur_id
    and m.motorisation_id = v.motorisation_id
    and v.statut_id != 99
    and v.vehicule_id = :vehicule_id
order by date_demande desc";
$stm = $pdo->prepare($sql); 
$stm->bindParam('vehicule_id', $cotid);
$stm->execute();
$vehicule_row = $stm->fetch();
file_put_contents($logfilew,$sql."\r\n",FILE_APPEND);
$date=date('d/m/y');
...
<tr width='800' bgcolor='#cccccc'><td height='50' width='400' align='center'>Site : {$vehicule_row[1]} </td><td td width='300'></td><td width='200'  align='center'> Vendeur : {$vehicule_row[2]}  </td></tr>
<tr><td width='900' colspan='3'>
	<table border='1' cellspan='0' cellspacing='0'>
		...
		<tr bgcolor='#eeeeee' ><td height='50' width='300' align='center'>{$vehicule_row[6]}</td><td width='300' align='center'>{$vehicule_row[8]}<br>{$vehicule_row[9]}<br>{$vehicule_row[10]}</td><td  width='300' align='center' bgcolor='#F7FF3C'>{$vehicule_row[29]} &euro;</td></tr>
		...
		<tr bgcolor='#eeeeee' ><td height='50' width='300' align='center'>{$vehicule_row[17]}</td><td width='300' align='center'></td><td  width='300' align='center' >{$vehicule_row[14]} </td></tr>
...
";
	...
	$email->Subject   = 'Offre AUTOBIZ Immatriculation : '.$vehicule_row[6] .' dossier : '.$id;
	$email->Body      = "Bonjour,\n ci-joint l'offre AUTOBIZ\nBonne Journee\n\n\n";
	//echo "Email :".$vehicule_row[26];
	$email->AddAddress( $vehicule_row[26]);
	$file_to_attach = $dirdoc.$cotid.'/16_offre_'.$cotid.'_'.$timestamp.'.pdf';
	$email->AddAttachment( $file_to_attach , 'offre.pdf' );
	$email->Send();
	$sql="insert into commentaires (vehicule_id,commentaire_desc, date_commentaire,commentaire_type,vendeur_id ) values ({$vehicule_row[0]},'Envoie Offre AUTOBIZ\n\n',now(),'vendeur',{$row1[5]});";
	$stm=$pdo->query($sql) ; 
...
if ( $vehicule_row[0] != '')
		...
		$email->Body      = "Bonjour,\nVotre mot de passe est : {$vehicule_row[0]} \n\n Bonne Journee\n\n CAP VO";
		$email->AddAddress( 'couverture@cap-vo.fr' );
...
foreach ( $stm->fetchAll() as $vehicule_row )
	{ 
	$check='';
	if ( $value == $vehicule_row[1] ) { $check ='selected';}
	if ( $type == 'statut' )	{ $dis='disabled';
								if ( $ofbca < $ofcapvo && $vehicule_row[1] == 4 ) { $dis =''; }
								if ( $vehicule_row[1] == 1 ) { $dis =''; $vehicule_row[0]='Demande Cotation CAPVO'; }
							
								//if ( $ofbca >= $ofcapvo && $vehicule_row[1] == 5 ) { $dis ='';}
								//if ( $vehicule_row[1] == 6 ) { $dis ='';}
								$str.="<option value='{$vehicule_row[1]}' $check $dis>{$vehicule_row[0]}</option>"; 
								}
	else {	$str.="<option value='{$vehicule_row[1]}' $check>{$vehicule_row[3]} - "  .$vehicule_row[2]." - "  .$vehicule_row[0]."</option>"; }
	}
...
$stm=$pdo->query($sql);
$str="";
foreach ( $stm->fetchAll() as $type_row )
	{ 
	$check='';
	if ( $value == $type_row[1] ) { $check ='selected';}
	if ( $type == 'statut' )	{ $dis='disabled';
								if ( $ofbca < $ofcapvo && $type_row[1] == 4 ) { $dis =''; }
								if ( $type_row[1] == 1 ) { $dis =''; $type_row[0]='Demande Cotation CAPVO'; }
							
								//if ( $ofbca >= $ofcapvo && $type_row[1] == 5 ) { $dis ='';}
								//if ( $type_row[1] == 6 ) { $dis ='';}
								$str.="<option value='{$type_row[1]}' $check $dis>{$type_row[0]}</option>"; 
								}
	else {	$str.="<option value='{$type_row[1]}' $check>{$type_row[0]}</option>"; }
	}
...
$sql1="insert into vehicule_couverts (
		concession_id,
		coteur_id,
		vendeur_id,
		statut_id,
...
$sql2="select max(vehicule_id) from  vehicule_couverts where concession_id='{$vehicule_row[2]}' and vendeur_id='{$vehicule_row[3]}' ;";
$stm2=$pdo->query($sql2) ; 
$newest_couverture_row = $stm2->fetch(); 
$d=date("Ymd-G:i:s");
file_put_contents($logfilew,$d." : Creation dossier N&deg;: {$newest_couverture_row[0]} par le vendeur : {$vehicule_row[0]} de la concession {$vehicule_row[2]}\r\n",FILE_APPEND);

// creation du dossier de sotckage des documents pour la cotation
$uploaddir = $dirdoc.$newest_couverture_row[0];
...
<input id=\"cotid\" name=\"cotid\" type=\"hidden\" value= \"{$newest_couverture_row[0]}\">
...
<tr><td height='35' width='224'>N&deg; Dossier : </td><td width='188'>{$newest_couverture_row[0]}</td></tr> 
...
$form_images = array(
	"avant" => array(
		"title" => "Photo Avant",
		"photo_name" => "front-car.png"
	),
	"droite" => array(
		"title" => "Photo Droite",
		"photo_name" => "right-car.png"
	),
	"arriere" => array(
		"title" => "Photo Arriere",
		"photo_name" => "rear-car.png"
	),
	"gauche" => array(
		"title" => "Photo Gauche",
		"photo_name" => "left-car.png"
	),
	"int1" => array(
		"title" => "Photo Int 1",
		"photo_name" => "interior-1.png"
	),
	"int2" => array(
		"title" => "Photo Int 2",
		"photo_name" => "interior-2.png"
	),
	"cg" => array(
		"title" => "Carte Grise",
		"photo_name" => "carte-grise.png"
	),
	"ct" => array(
		"title" => "Cont Tech",
		"photo_name" => "controle-technique.png"
	),
	"rep" => array(
		"title" => "Estimation",
		"photo_name" => "estimation.png"
	),
	"car1" => array(
		"title" => "Divers 1",
		"show_next" => true,
		"photo_name" => "divers.png"
	),
	"car2" => array(
		"title" => "Divers 2",
		"show_next" => true,
...
?>
	...
	(isset($infos['hidden']) && $infos['hidden']) ? "hidden hidden-by-" . $infos['hidden'] : ""); ?>
            <tr class="<?= $tr_classes ?>">
                <td height="45">
                    <?= $infos["title"] ?>: &nbsp;
                    <button type="button" class="BoutonPhoto hidden style-bouton" data:field_id="<?= $id ?>"
                        id="<?= $id ?>-bt_photo">
                    </button>
                </td>
                ...
                <td align="center">
                    <label for="photo_<?= $id ?>" class="label-file">
                        <!--&nbsp;&nbsp;&nbsp;&nbsp;<img id="Photo_loading_<?= $id ?>" src="/capvo/img/bouton-loading/app_photo.png" height="40px">-->
                        &nbsp;&nbsp;&nbsp;&nbsp;<img class="HoverZoom" id="Photo_loading_<?= $id ?>"
                            src="/dcapvo/img/bouton-loading/<?= $infos["photo_name"]?>" width="auto" height="60px">
                    </label>
                    <input type="file" id="photo_<?= $id ?>"
                        class="input-file image-input <?= (isset($infos['show_next']) && $infos['show_next']) ? "show-next" : "" ?>"
                        accept="image/*" style="display:none" name="photo_<?= $id ?>" />
                    </input>
                </td>
            </tr>
            <tr class="<?= $tr_classes ?>">
            ...
            <tr class="<?= $tr_classes ?>">
                <td colspan="2">
                    <div class="Previsualisation NotLoaded">
                    </div>
                </td>
            </tr>
...
<tr><td colspan='2'><input type='submit' name='Soumettre' disabled='true'  id='Soumettre' value='Soumettre la demande' onClick=\"sendSock('insert','{$newest_couverture_row[0]}')\"></input></td></tr>
