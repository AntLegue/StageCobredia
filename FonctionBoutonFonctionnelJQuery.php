...
$(document).ready(function()
	...
	function update_preview(event) {
		...
		$("tr.block_" + input_id + " button.BoutonPhoto").html("Fermer la photo");
		...
		//Ajout et retrait de class vérifiant si une image est chargée ou non
	...
	function replay_photo(event){
		...
		//Modifie le button_id pour mettre la première lettre en majuscule
		const button_id_Maj = button_id.charAt(0).toUpperCase() + button_id.slice(1);
		
		//id de la div a afficher
		var div_image = $("tr.block_photo_" + button_id + " div.Previsualisation");
		
		//id du bouton fermer a afficher
		var boutton_close = $("tr.block_photo_" + button_id + "bouton");
		
		//Vérifie si la div correspondante au bouton cliqué contient
		if(div_image.hasClass("NotLoaded")){
			
			//indique a l'utilisateur qu'aucune image n'est chargée
			window.alert("Impossible: Aucune image charg\u00e9e !");
		...
		if((div_image.css("display") == "none") && (div_image.hasClass("Loaded"))){
			...
			$("tr.block_photo_" + button_id + " button.BoutonPhoto").html("Fermer la photo");
			...
			//affiche "Afficher Photo" + button_id sur le bouton
			$("tr.block_photo_" + button_id + " button.BoutonPhoto").html("Afficher Photo " + button_id_Maj);
		};
	};
...
$form_images = array(
	...
	"car1" => array(
		"title" => "Probleme Divers 1"
	),
	"car2" => array(
		"title" => "Probleme Divers 2"
	),
	"car3" => array(
		"title" => "Probleme Divers 3"
	),
	"car4" => array(
		"title" => "Probleme Divers 4"
	),
	"car5" => array(
		"title" => "Probleme Divers 5"
	),
	"car6" => array(
		"title" => "Probleme Divers 6"
	),
	"car7" => array(
		"title" => "Probleme Divers 7"
	),
	"car8" => array(
		"title" => "Probleme Divers 8"
	),
	"car9" => array(
		"title" => "Probleme Divers 9"
	),
	"car10" => array(
		"title" => "Probleme Divers 10"
	)
);

// document.querySelectorAll(".block_" + split('-', this.id)[0] + " button.BoutonPhoto")

?>

<?php foreach($form_images as $id => $infos): ?>

<tr class="block-image-field block_photo_<?= $id ?>">
	<td height="45"><?= $infos["title"] ?> : </td>
	<td align="center">
		<label for="photo_<?= $id ?>" class="label-file">
			&nbsp;&nbsp;&nbsp;&nbsp;<img src="/capvo/img/app_photo.png"
			height="40px">
		</label>
	<input type="file" id="photo_<?= $id ?>" class="image-input"
...
<tr class="block-image-field block_photo_<?= $id ?>">
	<td colspan="2" >
		<button type="button" class="BoutonPhoto" data:field_id="<?= $id ?>" id="<?= $id ?>-bt_photo">
			Afficher <?= $infos["title"] ?>
		</button>
	</td>
</tr>
